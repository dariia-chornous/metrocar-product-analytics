...
funnel_with_prev AS (
    SELECT
        ...
        step_order,
        users,
        LAG(users) OVER (ORDER BY step_order) AS users_previous_step
    FROM funnel
)
SELECT
    funnel_step,
    users,
    users_previous_step,
    (users_previous_step - users) AS users_lost,
    ROUND(users::numeric / NULLIF(users_previous_step, 0), 3) AS conversion_rate
